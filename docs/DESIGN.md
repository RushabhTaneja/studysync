...
| **SMART-on-FHIR is the proven-genuine integration** | Its public sandbox needs no app registration, so it runs immediately and end-to-end. Dexcom requires self-serve developer registration; the full OAuth + EGV-ingest code is complete and credential-gated, activating the moment `DEXCOM_CLIENT_ID/SECRET` are set. |
